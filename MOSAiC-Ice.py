...
class Buoy:
    ...
    def processbuoy(self):
        ...
        if dftype == 'Full Dataset':
            ...
            keymetrics = tt[['Date','Air_Temp_(C)','Th(snow)','Th(ice)','Th(ocean)']]
            st.dataframe(keymetrics)

        def get_table_download_link(tt):
            """Generates a link allowing the data in a given panda dataframe to be downloaded
            in:  dataframe
            out: href string
            """
            csv = tt.to_csv(index=False)
        ...
        st.markdown(get_table_download_link(tt), unsafe_allow_html=True)
